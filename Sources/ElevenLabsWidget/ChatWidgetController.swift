...
///
/// Optional — pass one to ``ChatWidget/init(authProvider:widgetConfig:conversationConfig:controller:onClientToolCall:)``
/// to read state through the `@Published` mirrors and to issue commands.
///
...
public final class ChatWidgetController: ObservableObject {
    ...
    public func endConversation() async {
        await binding?.client.endConversation()
    }
