...
public struct ConversationConfig: Sendable {
    ...
    /// Opt-in event-based agent-state tracking (VAD + client events). When set,
    /// `ConversationClient.agentState` is driven by this heuristic; when `nil`,
    /// agent state is derived from the transport's speaking detection.
