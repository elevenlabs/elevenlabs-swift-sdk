...
public final class ConversationClient: ObservableObject {
    ...
    public func startConversation(
        ...
        try await startConversation(auth: authConfig, config: updatedConfig)
    }
    ...
    public func startConversation(
        ...
        bind(conversation)
        try await conversation.startConversation(auth: auth, config: config)
    }
    ...
    private func bind(_ session: Conversation) {
        ...
        session.$startupState.sink { [weak self] in self?.startupState = $0 }.store(in: &cancellables)
        session.$startupMetrics.sink { [weak self] in self?.startupMetrics = $0 }.store(in: &cancellables)
        ...
        session.$isMuted.sink { [weak self] in self?.isMuted = $0 }.store(in: &cancellables)
        ...
        session.$latestAudioAlignment.sink { [weak self] in self?.latestAudioAlignment = $0 }.store(in: &cancellables)
        session.$latestAudioEvent.sink { [weak self] in self?.latestAudioEvent = $0 }.store(in: &cancellables)
    }
    ...
    public func setMicrophoneMuted(_ muted: Bool) async throws {
        try await session?.setMicrophoneMuted(muted)
    }
