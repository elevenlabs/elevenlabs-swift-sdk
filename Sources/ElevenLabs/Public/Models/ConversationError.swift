...
public enum ConversationError: LocalizedError, Sendable, Equatable {
    ...
    /// A multimodal message had neither text nor a file attachment.
    case invalidMultimodalMessage
    ...
    case invalidURL
    /// A conversation-scoped REST endpoint failed (non-2xx or unreachable). The
    /// associated value carries a human-readable detail.
    ...
    public var errorDescription: String? {
        ...
        case .invalidMultimodalMessage: "A multimodal message must include text or a file."
        case let .invalidToolResult(detail): "Invalid tool result: \(detail)"
        case .invalidURL: "Could not build a valid request URL."
        case let .fileUploadFailed(detail): "File upload failed: \(detail)"
        case let .fileDeleteFailed(detail): "File delete failed: \(detail)"
        case let .feedbackSubmissionFailed(detail): "Feedback submission failed: \(detail)"
        }
    }
