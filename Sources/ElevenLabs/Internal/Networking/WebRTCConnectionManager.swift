...
/// Façade around `LiveKit.Room`: owns the room lifecycle, microphone control,
/// data publish/receive, and surfaces typed callbacks (connection state, remote
/// speaking, remote disconnect) plus an async readiness API. Centralizes the
/// LiveKit dependency — LiveKit types never cross the public boundary.
///
/// LiveKit observation is split across two `RoomDelegate`s:
/// - `LiveKitRoomEventDelegate` — data, speaking, remote disconnect
/// - `LiveKitReadinessDelegate` — first remote participant joined
@MainActor
final class WebRTCConnectionManager: WebRTCConnectionManaging {
    ...
    /// so callers can (re)attach audio renderers to the current tracks.
...
final class LiveKitRoomEventDelegate: RoomDelegate {
    ...
    // Audio track availability changes — used to (re)attach audio renderers.
