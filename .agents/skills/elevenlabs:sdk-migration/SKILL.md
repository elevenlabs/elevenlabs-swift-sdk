...
Hosts managing their own `AVAudioSession` can set `AudioPipelineConfiguration.recordingAlwaysPrepared` to `false`; it defaults to `true` and is disabled when the conversation ends.
...
`complete(_:with:)` sends the result when `call.expectsResponse` is true; otherwise it calls `markToolCallCompleted` without sending. `result` accepts a `String` directly or any `Encodable` (JSON-encoded, throwing initializer). Where the app distinguishes kinds of tool failure, suggest the `errorType:` parameter (`.userRejected`, `.clientTimeout`, `.externalServer`, ...).
...
- **Leave-during-connect workarounds.** v4 tears startup down cleanly at any stage: cancel the `Task` running the start, or call `endConversation()` while connecting. An already-cancelled start task is rejected before it can replace the live session.
