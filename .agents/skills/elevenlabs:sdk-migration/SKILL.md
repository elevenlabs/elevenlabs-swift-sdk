...
- Startup failures throw `ConversationStartupError`, which carries the failed stage and the underlying `ConversationError`. Existing `catch let error as ConversationError` or `as? ConversationError` around a start still compiles but never matches: catch `ConversationStartupError` and read `.underlyingError`.
...
v3's `AudioPipelineConfiguration.recordingAlwaysPrepared` is now `preparesMicrophone`, a non-optional `Bool`. It defaults to `true`: the mic pipeline is prepared while a voice conversation is starting or live and released when it ends. `false` only skips that warm-up; LiveKit still configures the audio session during calls and deactivates it when a call ends.
